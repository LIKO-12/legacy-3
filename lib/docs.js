...
import navbarConfiguration from "../docs/_navbar.json";
...
export async function getDocumentData(id) {
    ...
    return {
        id,
        contentHtml,
        links: getDocumentNavbarLinks(id),
        ...matterResult.data
    }
}
...
export function getDocumentNavbarLinks(id) {
    const links = [];

    for (let link of navbarConfiguration) {
        const entry = { name: link[0], absolute: link[1] };

        if (link[1]) entry.href = "/" + link[1];
        else entry.disabled = true;

        links.push(entry);
    }

    // let dirName = path.posix.dirname(id);
    // outer_loop:
    // while (dirName != ".") {
